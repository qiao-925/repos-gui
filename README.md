...
提供可视化界面，用于同步 GitHub 公共仓库到 REPO-GROUPS.md 的"未分类"分组。
...
4. **切换主题**：
   - 点击菜单栏"🎨 主题"
   - 选择"深色主题"或"浅色主题"
   - 选择喜欢的颜色方案（Teal、Blue、Cyan、Purple、Amber）

### 功能特点

- **预览优先**：先预览新增仓库列表，确认后再写入
- **异步处理**：使用后台线程，界面不会卡顿
- **实时日志**：显示操作日志，方便追踪执行过程
- **错误提示**：清晰的错误信息和建议操作
- **Material Design 主题**：使用 qt-material 提供专业的 Material Design 风格
- **多主题切换**：支持 10+ 种主题配色，深色/浅色随心切换

### 注意事项

- GUI 仅用于同步新增仓库，不处理删除/改名/转移
- 同步完成后，可以使用 `repos clone` 命令克隆新增仓库
- 如果不需要 GUI，可以使用 `repos sync` 命令行工具
